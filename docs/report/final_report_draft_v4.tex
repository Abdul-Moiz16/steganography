...
\subsection{Training Outcomes and Test-Set AUCs (V1)}
\label{sec:v1-train}

The V1 training corpus is the 3{,}000-caption source-balanced run of Section~\ref{sec:experiments} (real + SDXL + FLUX, 1:1:1; per-checkpoint $n_\text{train}\!=\!18{,}900$, $n_\text{val}\!=\!5{,}292$). All six checkpoints converged within budget. Validation AUCs: SRNet $\{0.9989, 0.9988, 0.9990\}$ at LSB / \{low, medium, high\}; DCTR $\{0.6174, 0.7949, 0.9363\}$ at DCT / \{low, medium, high\}. Applying the checkpoints as-is to the held-out test set (leakage-guarded by manifest hash; Section~\ref{sec:learned-detectors}) gives the per-source AUCs of Table~\ref{tab:learned-headline-auc}. SRNet saturates the LSB-detection task at AUC$\geq$0.997 across all carriers, payloads and encryption modes -- substantially above the strongest classical LSB detector (sample\_pairs at 0.93 overall). DCTR sits at AUC 0.81 overall, between $\chi^2$-DCT-tiled (0.85) and $\chi^2$-DCT (0.76); its per-cell range from 0.62 (low payload) to 0.94 (high payload) tracks the payload signal cleanly.

\begin{table}[!htbp]
\centering\footnotesize
\caption{V1 learned-detector AUC on the test run, per carrier source. ML-A$=$SDXL, ML-B$=$FLUX. Encryption averaged within each cell (per-encryption agree to $\leq\!0.001$). Overall: SRNet 0.998, DCTR 0.814.}
...
\begin{table}[!htbp]
\centering\footnotesize
\caption{V1 cross-validation verdicts. Classical and learned $\Delta_\text{AUC}$ inverse-variance pooled within each detector family.}
...
\begin{table}[!htbp]
\centering\footnotesize
\caption{V2a real-only training: per-(detector, payload) validation AUC and per-source test AUC at the plain-encryption cell. ``Val'' is the held-out-of-training-corpus AUC; the three source columns are AUC on the held-out test corpus (108{,}000 stegos $+$ covers). DCTR also carries an out-of-bag estimate $E_\text{OOB}$ of the minimum total detection error at equal priors, computed by the same FLD-ensemble convention as Holub \& Fridrich~\cite{holub2015dctr} Table~V.}
...
\begin{table}[!htbp]
\centering\footnotesize
\caption{V1 (1:1:1 real/SDXL/FLUX training) vs.\ V2a (real-only training) -- per-source test AUC at the plain-encryption cell, contrasted directly. The V1 numbers are the same predictions used in Table~\ref{tab:learned-headline-auc} (Section~\ref{sec:learnedxval}) and Figure~\ref{fig:within-family}. Bold entries mark drops $\geq 0.10$ from V1 to V2a -- the diagnostic of training-balance dependence.}
\label{tab:v1-vs-v2a}
\setlength{\tabcolsep}{3pt}\renewcommand{\arraystretch}{1.05}
\begin{tabular}{@{}ll | ccc | ccc@{}}
\toprule
& & \multicolumn{3}{c}{\textbf{V1 (balanced)}} & \multicolumn{3}{c}{\textbf{V2a (real-only)}} \\
...
\paragraph{V2a verdict: balanced-training artefact, decisively.} V2a SRNet retains near-perfect detection on the carrier source it was trained on (real, AUC 0.99 across all three payload levels) but \emph{collapses on the unseen AI sources}, dropping to AUC 0.55--0.71 on ML-A (SDXL) and 0.64--0.93 on ML-B (FLUX) -- a $0.26$--$0.45$~AUC gap relative to its same-payload performance on real photographs. The V1 SRNet per-source flatness (range $\sim$0.003 at every cell, Table~\ref{tab:v1-vs-v2a}) is therefore \emph{not} architectural source-invariance: it is a consequence of training on a 1:1:1 carrier mix that included the test-time AI sources. When the training mix is removed, the deep spatial CNN reveals a substantial real-vs-AI generalisation gap consistent with the cover-source-mismatch literature~\cite{giboulot2020csm,mallet2024csm,mereur2024deepfake}. DCTR shows the same pattern at smaller absolute magnitude: V1's small ML-easier ordering (ml\_a $>$ real by 0.02--0.04) flips under V2a to real-easier (real $>$ ml\_a/ml\_b by 0.02--0.09) and overall AUC drops on every cell, confirming that the V1 DCTR per-source pattern was also a balanced-training artefact rather than an intrinsic feature of the DCTR feature set.

\paragraph{Implication for RQ1.} Re-running the V1 cross-validation pipeline on the V2a predictions (\texttt{runs/.../learned\_shadow\_v2a/}) flips four of the five RQ verdicts at the learned-detector family level: RQ1 from trivial ($\Delta\!=\!+0.0012$) to supported with $\Delta\!=\!+0.2473$ (CI $[+0.245,+0.250]$); RQ2 from trivial to supported with $\Delta\!=\!-0.0689$; RQ4 (spatial vs.\ frequency branch contrast) from trivial to supported with $\Delta\!=\!+0.2195$; RQ3 retains the mixed verdict but the per-payload real-minus-ML gaps grow from $\{-0.014, -0.013, -0.007\}$ to $\{+0.053, +0.220, +0.213\}$. RQ5 (encryption invariance) is unchanged at $\Delta\!\approx\!0$ as expected -- an encryption ablation does not depend on training-set carrier composition.

\paragraph{Mechanism of the V2a medium/high-payload collapse on ML carriers.} V2a SRNet on ML carriers is worst at medium payload (AUC 0.55 on ML-A) and recovers only partially at high payload (0.59), rather than improving monotonically with payload as the standard expectation predicts. A per-(payload, source, label) decomposition of the SRNet score distribution explains this directly: at medium and high payload, V2a SRNet assigns ML \emph{covers} a near-saturated stego score (cover-mean 0.79--0.95) almost indistinguishable from the score it assigns ML \emph{stegos} (stego-mean 1.000); cover-stego separation collapses to a shift of 0.05--0.27 on the [0,1] score axis and AUC drops accordingly. On real carriers the same checkpoint behaves normally: cover-mean 0.030, stego-mean 0.978, shift 0.95. The V2a-medium checkpoint was trained to discriminate ``real cover'' from ``real cover $+$ 15\,\% LSB perturbation,'' and at test time it interprets ML covers' diffusion-decoder noise patterns as if they were the perturbation signal it was trained to detect. The same effect appears more weakly at high payload (cover-mean 0.91 on ML-A) and is largely absent at low payload (cover-mean 0.16 on ML-A) -- the lower-payload checkpoint's tighter decision boundary still admits separation. This is structurally consistent with the cover-side variance-inflation mechanism we identify for the $\chi^2$-spatial detector (Section~\ref{sec:chi2spatial}): both detectors register diffusion-decoder noise as a stego-like signal on the cover side, and both consequently lose discrimination power on ML carriers in ways the same detectors applied to real carriers do not. A finer-grained analysis of which SRNet layer activations drive this is a natural follow-up.
...
\paragraph{Revised RQ1 reading.} The carrier-source effect of RQ1 is real and reproducible \emph{when the detector has room to register it}. The DCT family unambiguously sees real-as-harder across three classical detectors and one learned detector. The spatial family sees real-as-harder across two classical detectors; the third classical detector ($\chi^2$-spatial) reverses this and the reversal dissolves under pAUC (Section~\ref{sec:chi2spatial}); under V1 (balanced training) SRNet abstracts the signal away. The corrected academic claim is therefore: \emph{the carrier-source effect of RQ1 is a property of the detector family, not the carrier alone; it is observable wherever a detector has both genuine sensitivity to the embedding distortion and headroom above the ceiling}. The V2a ablation (\S\ref{sec:v2a}) further establishes that V1 SRNet's apparent per-source flatness was a balanced-training artefact, not an architectural property of the deep CNN: when SRNet is trained on real photographs only, the real-vs-ML AUC gap re-emerges at $+0.247$ pooled and up to $+0.44$ at the medium/high-payload cells -- a dramatic confirmation that the deep spatial detector does \emph{not} intrinsically generalise across carrier sources without seeing them at training time.
...
\paragraph{Learned-detector caveats.} Three caveats accompany the supplementary learned-detector analysis. \textit{(i)} A single training-set random seed (\texttt{4242}) is used for both V1 and V2a; we do not report multi-seed variability of the learned-AUC estimates. The headline V2a finding -- 0.26--0.45 AUC drop on AI carriers when SRNet is trained real-only -- rests on a large, mechanistically explained structural effect (\S\ref{sec:v2a}: cover-side score saturation on ML carriers when the trained checkpoint interprets diffusion noise as LSB perturbation) and is not expected to be seed-dependent in direction; the absolute magnitude may shift by a few hundredths across seeds. A three-seed re-run of V2a SRNet would tighten the AUC point estimates with error bars but would not change the qualitative conclusion. \textit{(ii)} The DCTR feature extractor uses the 8{,}000-dim variant of Holub \& Fridrich~\cite{holub2015dctr} interpreted as 64 modes $\times$ 25 phases (5$\times$5 alignment subset) $\times$ 5 truncated bins; the exact phase decomposition that yields the paper's headline 8{,}000 count is somewhat ambiguous from the paper text alone, and our interpretation is one of several defensible reconstructions. \textit{(iii)} SRNet is trained on caption-matched data, not on BOSSBase; this is the academically preferable choice for our cover-source-mismatch-aware design, but it makes our AUC numbers not directly comparable to literature SRNet figures on BOSSBase.
