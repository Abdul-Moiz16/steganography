...
\begin{figure}[!htbp]
\centering
\includegraphics[width=\linewidth]{figures/v4_paper/v1_vs_v2a_heatmap.png}
\caption{Per-(detector, payload, source) test AUC under matched (V1, left) vs real-only (V2a, right) training. Cells are coloured by AUC (red $\sim$0.5, green $\sim$1.0). Under matched training every cell is high; under real-only training the SRNet ml\_a / ml\_b cells at medium and high payload collapse to red (AUC 0.55--0.71), while the real column stays green (AUC 0.99). DCTR shows the same pattern at smaller absolute magnitude. The heatmap form makes the cover-source-mismatch effect visually immediate: red blocks materialise only in the columns the model never saw at training time.}
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.82\linewidth]{figures/v4_paper/dctr_eoob_ladder.png}
\caption{DCTR under real-only training: three error metrics by payload. $E_\mathrm{OOB}$ (dark) is the training-time out-of-bag ensemble residual (Holub \& Fridrich~\cite{holub2015dctr} convention, computed post-training by deterministic re-fit). Test-set $P_E^{\min}$ (orange) is the operational detection error on the held-out test corpus (real + ML carriers). $1-\mathrm{val\_AUC}$ (light) is a held-out-of-training proxy for $P_E$ on the in-distribution carriers (real only). Test-set $P_E^{\min}$ is uniformly worse than $E_\mathrm{OOB}$ because the test corpus contains ML carriers that the real-only-trained DCTR never saw -- the cover-source-mismatch effect manifest at the operational metric.}
\label{fig:dctr-eoob-ladder}
\end{figure}

\begin{figure}[!htbp]
\centering
\includegraphics[width=\linewidth]{figures/v4_paper/v2a_srnet_score_dists.png}
\caption{Real-only-trained SRNet score distributions in a $3\!\times\!3$ grid (rows: payload; columns: source). The Real column behaves as a clean cover-vs-stego binary classifier with shift $\approx 0.95$ between distributions. The SDXL and FLUX columns at medium and high payload exhibit \emph{cover-saturation}: the model assigns ML covers a near-1.0 stego score because their diffusion-decoder noise patterns share structural features with the LSB perturbations the model was trained to detect; cover-stego separation collapses to a shift of 0.05--0.27 and AUC drops accordingly. This mechanism is structurally consistent with the $\chi^2$-spatial cover-side variance-inflation of Section~\ref{sec:chi2spatial}: both detectors register diffusion-decoder noise as a stego-like signal on the cover side.}
\label{fig:v2a-srnet-mechanism}
\end{figure}

\begin{figure}[!htbp]
\centering
\includegraphics[width=\linewidth]{figures/v4_paper/verdict_matrix.png}
\caption{Per-RQ verdict matrix across the three detector-family analyses: classical (6 detectors), matched-training learned (V1; SRNet + DCTR), real-only-training learned (V2a; SRNet + DCTR).  Verdicts are colour-coded (orange = supported; blue = mixed; grey = trivial) with the pooled $\Delta_{AUC}$ in each cell.  Four of five RQ verdicts flip under real-only training: RQ1 trivial$\to$supported with $\Delta\!=\!+0.247$, RQ2 trivial$\to$supported with $\Delta\!=\!-0.069$, RQ4 trivial$\to$supported with $\Delta\!=\!+0.220$.  RQ5 (encryption invariance) is unchanged across all three analyses ($\Delta\!\approx\!0$) as expected -- training-distribution changes do not affect what encryption does to the embedding.}
\label{fig:verdict-matrix-three-way}
\end{figure}

\paragraph{Implication for RQ1.} Re-running the cross-validation pipeline on the real-only predictions (\texttt{runs/.../learned\_shadow\_v2a/}) flips four of the five RQ verdicts at the learned-detector family level: RQ1 from trivial ($\Delta\!=\!+0.0012$) to supported with $\Delta\!=\!+0.2473$ (CI $[+0.245,+0.250]$); RQ2 from trivial to supported with $\Delta\!=\!-0.0689$; RQ4 (spatial vs.\ frequency branch contrast) from trivial to supported with $\Delta\!=\!+0.2195$; RQ3 retains the mixed verdict but the per-payload real-minus-ML gaps grow from $\{-0.014, -0.013, -0.007\}$ to $\{+0.053, +0.220, +0.213\}$. RQ5 (encryption invariance) is unchanged at $\Delta\!\approx\!0$ as expected -- an encryption ablation does not depend on training-set carrier composition.  Figure~\ref{fig:verdict-matrix-three-way} renders the full classical / matched / real-only verdict matrix.
