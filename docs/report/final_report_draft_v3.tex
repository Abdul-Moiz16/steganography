...
Table~\ref{tab:learned-verdicts} reports the learned-detector verdict for each RQ alongside the corresponding classical-detector verdict, with the three-way interpretation \textbf{corroborates / refines / contradicts}.

\begin{table}[h]
\centering\small
\caption{V1 learned-detector cross-validation of the pre-registered classical-detector verdicts. ``Corroborates'' = same verdict, same direction; ``Refines'' = same direction, different magnitude; ``Contradicts'' = opposite direction. Pooled $\Delta_\text{AUC}$ is inverse-variance-weighted over the six learned-detector strata.}
...
RQ5 (encryption invariance)       & $\approx 0$ (supported) & $\approx 0$ (supported, 18/18 within $\pm 0.025$) & {\bfseries Corroborates} (preserves methodological assumption) \\
\bottomrule
\end{tabular}
\end{table}

The single ``contradicts'' verdict (RQ1) deserves the unpacking in Section~\ref{sec:disc-rq1-revisited}: it is driven entirely by inverse-variance pooling weighting saturated SRNet at near-infinite weight and effectively ignoring DCTR -- whose per-source pattern actually \emph{corroborates} the classical RQ1 in direction and magnitude.

\subsection{Within-Family Fair Comparison at Low Payload}
\label{sec:within-family}

Each detector scores only its own embedding branch, so within each branch we have a clean four-way comparison (3 classical + 1 learned) on identical test cells. Table~\ref{tab:within-family} shows the per-source AUC at the low-payload + plain-encryption cell, where headroom is largest.

\begin{table}[h]
\centering\small
\caption{Within-family per-source AUC at the low-payload, plain-encryption cell (largest detection-headroom regime; $n\!=\!6{,}000$ stegos $+$ $6{,}000$ covers per cell). $\Delta\!=\!$ real $-$ pooled ML. Detectors with overall AUC $<\!0.6$ in this regime are flagged ``near-random'' -- their tiny source range is not informative about source-sensitivity.}
\label{tab:within-family}
\setlength{\tabcolsep}{4pt}\renewcommand{\arraystretch}{1.1}
\begin{tabular}{@{}lcccrr@{}}
\toprule
\textbf{Detector} & \textbf{Real} & \textbf{ML-A} & \textbf{ML-B} & \textbf{Range} & \textbf{$\Delta_\text{real-pmL}$} \\
\midrule
\multicolumn{6}{l}{\textit{LSB family (low payload, plain)}} \\
rs                       & 0.856 & 0.941 & 0.952 & 0.096 & $-0.091$ \\
sample\_pairs            & 0.883 & 0.974 & 0.971 & 0.091 & $-0.089$ \\
$\chi^2$-spatial         & 0.541 & 0.530 & 0.533 & 0.012 & $+0.010$ {\scriptsize (near-random)} \\
\textbf{SRNet (V1)}      & \textbf{0.999} & \textbf{0.997} & \textbf{0.998} & \textbf{0.003} & \textbf{$+0.002$} \\
\midrule
\multicolumn{6}{l}{\textit{DCT family (low payload, plain)}} \\
$\chi^2$-DCT             & 0.581 & 0.620 & 0.605 & 0.039 & $-0.031$ \\
$\chi^2$-DCT-tiled       & 0.635 & 0.702 & 0.724 & 0.089 & $-0.078$ \\
calibration-$\chi^2$     & 0.519 & 0.528 & 0.531 & 0.012 & $-0.011$ {\scriptsize (near-random)} \\
\textbf{DCTR (V1)}       & \textbf{0.605} & \textbf{0.645} & \textbf{0.623} & \textbf{0.041} & \textbf{$-0.030$} \\
\bottomrule
\end{tabular}
\end{table}

Two patterns emerge from Table~\ref{tab:within-family}:
\begin{itemize}
\item \textbf{DCTR sits squarely in its family.} Its per-source range (0.041), ordering (ML-A $>$ ML-B $>$ real), and signed gap ($-0.030$) are all bracketed by the working classical DCT detectors. The learned frequency detector replicates the classical RQ1 finding within the DCT branch.
\item \textbf{SRNet is the LSB-family outlier.} The two working classical LSB detectors show source range $\sim$0.09 with strong real-harder direction; SRNet shows range 0.003 and a near-zero gap. The categorical difference cannot be dismissed as ``the task was too easy'' because the same task is hard for SRNet's classical peers (best classical AUC at this cell is 0.93, not near ceiling). However, SRNet at 0.999 AUC has so little residual error budget that a ``source-correlated errors absorbed into the residual'' interpretation is also consistent with the data. The V2a ablation (Section~\ref{sec:v2a}) is designed to distinguish these two interpretations.
\end{itemize}
...
\begin{table}[h]
\centering\small
\caption{V2a real-only training: validation AUCs and per-source test-set AUCs (low-payload, plain-encryption cell, mirroring Table~\ref{tab:within-family}).  Numbers landed at the time of this draft are filled in; \protect\TBD{cells} mark stages whose results we are still post-processing.}
