...
function buildConditionsTab(data) {
    ...
    var body = detectors.map(function (detector) {
        ...
        var detailHeader = `<th></th>` + columns.map(function (col) {
            return `<th style="text-align:center;font-size:9px;color:var(--secondary-dim);font-weight:600">${escapeHtml(col.method.toUpperCase())} / ${escapeHtml(col.level)} / ${escapeHtml(col.encryption)}</th>`;
        }).join('');
