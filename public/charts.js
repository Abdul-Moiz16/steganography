/* ═══════════════════════════════════════════════════════════════════════════
   Stego Explorer — Canvas chart helpers (M3 dark theme)
   ═══════════════════════════════════════════════════════════════════════════ */
...
function drawHorizontalBars(canvas, labels, values, colors) {
    ...
    var W = canvas.parentElement.clientWidth || 500, H = canvas.height || 200;
    canvas.width = W * dpr; canvas.height = H * dpr;
    canvas.style.width = W + 'px'; canvas.style.height = H + 'px';
    var ctx = canvas.getContext('2d'); ctx.scale(dpr, dpr);

    var lw = Math.max(100, labels.reduce(function (m, l) { return Math.max(m, l.length * 7); }, 0));
    var pad = { t: 10, r: 64, b: 10, l: lw };
    var cw = W - pad.l - pad.r, n = labels.length, gh = H / n, bh = Math.min(20, gh - 12);
    ...
    labels.forEach(function (lbl, i) {
        var cy = pad.t + i * gh + gh / 2, v = Math.max(0, Math.min(1, values[i])), bw = v * cw;
...
function drawGroupedBars(canvas, labels, datasets) {
    ...
    var bw = Math.min(24, Math.max(6, (gw - 14) / nd));
    ctx.clearRect(0, 0, W, H);
