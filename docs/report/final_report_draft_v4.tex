...
\noindent\textbf{Abstract}---This article examines whether the source of a carrier image -- a real photograph or a diffusion-generated synthetic -- alters the detectability of least-significant-bit steganography under fixed embedding settings. A caption-matched factorial corpus of 3{,}000 cover-groups across three carrier sources (real, SDXL, FLUX.1-schnell), two embedding branches (spatial LSB on PNG, JSteg-style DCT-LSB on JPEG~Q=95), three payload levels, and two payload conditions (plain and AES-256-CBC) is scored by six training-free statistical detectors and analysed with paired DeLong tests under Bonferroni--Holm correction. Five of the six detectors find diffusion carriers easier to attack than photographs; the sixth ($\chi^2$-spatial) reverses, an effect that vanishes under partial-AUC at operational false-positive rates. The pre-registered protocol is then cross-validated on a methodologically orthogonal detector family: a deep residual network (SRNet) and an undecimated-DCT feature extractor with an LDA ensemble (DCTR), both trained on a caption-matched corpus disjoint from the test set in two configurations: \textbf{matched training} (training-mix matches test-mix, 1:1:1 real/SDXL/FLUX) and \textbf{real-only training} (AI carriers held out from training; the realistic deployment scenario). The matched-training cross-validation replicates four of five findings; the matched-vs-real-only contrast establishes that SRNet's apparent source-invariance under matched training is a training-mix artefact rather than architectural -- removing AI carriers from training opens a $0.26$--$0.45$~AUC gap on the same AI test carriers, confirming the carrier-source effect at the deep-CNN level under the realistic deployment regime. Two additional contributions are reported: a tile-local Westfeld $\chi^2$ variant that becomes the strongest frequency-branch classical detector on our test set, and a mechanistic account of the $\chi^2$-spatial reversal grounded in diffusion-decoder tone-curve smoothness.
...
For two decades, the published benchmarks have implicitly assumed the cover is a photograph carrying the noise pattern and JPEG history of a real camera pipeline. Latent-diffusion models such as Stable Diffusion XL~\cite{sdxl2023} and FLUX.1~\cite{flux2024} now produce photorealistic images at scale through a process that contains none of the sensor noise, demosaicing, or in-camera JPEG steps a detector would expect. Corvi~\textit{et~al.}~\cite{corvi2023diffusion} reported that diffusion outputs leave their own characteristic spectral fingerprints, and a small recent literature on cover-source mismatch for synthetic carriers~\cite{mereur2024deepfake,mallet2024csm} has begun to address whether classical steganalysis remains effective when the cover is generated rather than captured. The literature so far has not, however, evaluated the question at the level of detector-pair contrasts on a controlled factorial corpus.

This article addresses that gap. We construct a caption-matched factorial corpus spanning three carrier sources, two embedding branches, three payload levels, and two payload conditions (plain and AES-256-CBC) at $N\!=\!3{,}000$ caption-groups, evaluate six training-free statistical detectors under a pre-registered DeLong + Bonferroni--Holm protocol, and cross-validate every pre-registered finding on a methodologically orthogonal detector family (the deep residual network SRNet~\cite{boroumand2019srnet} and the undecimated-DCT feature set DCTR~\cite{holub2015dctr} with an LDA ensemble~\cite{fridrich2012srm}). Two further contributions arise from the analysis of individual detectors that under- or out-perform the field.

\paragraph{Contributions.} This work makes four contributions:
\begin{enumerate}
\item A caption-matched factorial corpus and a pre-registered, training-free six-detector evaluation of the carrier-source effect (real vs.\ diffusion-generated) on LSB-style steganography, with paired DeLong tests, Bonferroni--Holm correction, and a partial-AUC analysis at FPR\,$\leq\!0.10$ (Sections~\ref{sec:methods}, \ref{sec:results-classical}).
\item A cross-validation of every pre-registered finding on a methodologically orthogonal learned-detector family (SRNet, DCTR) trained on a disjoint corpus, run in two complementary configurations -- \textit{matched training} (1:1:1 real/SDXL/FLUX) and \textit{real-only training} (AI carriers held out). The matched-vs-real-only \emph{contrast} establishes that SRNet's apparent source-invariance under matched training is a training-mix artefact rather than an architectural property: removing AI carriers from training opens a $0.26$--$0.45$ AUC gap on AI test carriers (Section~\ref{sec:learnedxval}).
...
\paragraph{Bridge to cross-validation.} The pre-registered six-detector verdicts are complete. Section~\ref{sec:learnedxval} now re-runs the same DeLong + Bonferroni--Holm + $\delta_\text{min}$ protocol on a methodologically orthogonal detector family (two learned baselines, SRNet and DCTR) trained on a disjoint corpus, in two complementary configurations: \emph{matched training} (the pre-registered learned cross-validation) and \emph{real-only training} (an in-paper ablation that holds AI carriers out of training to address the only remaining ambiguity about how training-set composition shapes the learned-detector findings).
