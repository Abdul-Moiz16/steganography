...
\paragraph{Caveat -- non-monotone payload behaviour.} V2a SRNet on ML carriers is worst at medium payload (AUC 0.55 on ML-A) and recovers slightly at high payload (0.59 on ML-A) rather than improving monotonically with payload. This inverts the standard expectation that detection gets easier as more bits are embedded. A plausible explanation is that low-payload detection forces SRNet to learn generalisable noise-residual features (small absolute perturbation, must rely on universal stego-signature statistics) whereas at medium/high payload the V2a model can over-rely on content-correlated features specific to the natural-image carriers it was trained on, which then fail on the structurally different AI carrier distribution. A finer-grained analysis of which SRNet layer activations drive the V2a generalisation gap is outside the scope of this paper but is a natural follow-up.
