...
\item A cross-validation of every pre-registered finding on a methodologically orthogonal learned-detector family (SRNet, DCTR) trained on a disjoint corpus, with a real-only-training ablation that disentangles architectural source-invariance from balanced-training artefact in the deep spatial branch (Section~\ref{sec:learnedxval}).
...
\begin{table}[!htbp]
\centering\footnotesize
\caption{V2a real-only training: validation and per-source test AUCs at the low-payload, plain-encryption cell (compare Figure~\ref{fig:within-family}). \protect\TBD{} cells await the running pipeline.}
\label{tab:v2a-results}
\setlength{\tabcolsep}{4pt}\renewcommand{\arraystretch}{1.05}
\begin{tabular}{@{}llcccc@{}}
\toprule
\textbf{Det.} & \textbf{Cell} & \textbf{Val} & \textbf{Real} & \textbf{ML-A} & \textbf{ML-B} \\
\midrule
SRNet & low    & \TBD{a} & \TBD{a} & \TBD{a} & \TBD{a} \\
SRNet & medium & \TBD{a} & \TBD{a} & \TBD{a} & \TBD{a} \\
SRNet & high   & \TBD{a} & \TBD{a} & \TBD{a} & \TBD{a} \\
DCTR  & low    & \TBD{a} & \TBD{a} & \TBD{a} & \TBD{a} \\
DCTR  & medium & \TBD{a} & \TBD{a} & \TBD{a} & \TBD{a} \\
DCTR  & high   & \TBD{a} & \TBD{a} & \TBD{a} & \TBD{a} \\
\bottomrule
\end{tabular}
\end{table}

The ablation has a binary read: if SRNet-v2a AUC stays $\geq 0.95$ on ML-A/ML-B at test time, the deep CNN's source-invariance is architectural (it abstracts source structure away regardless of training exposure); if it drops substantially, source-invariance was a balanced-training artefact and the carrier-source signal exists in the spatial domain too. \TBD{Once V2a lands: one sentence specifying which outcome obtained and the corresponding interpretation of the V1 SRNet near-flat per-source AUC.}
