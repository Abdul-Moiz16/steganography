...
\item A training-free six-detector evaluation under the paired DeLong test with Bonferroni--Holm correction and a pre-registered practical-significance gate (Section~\ref{sec:results}).
\item A supplementary partial-AUC analysis at FPR\,$\leq\!0.10$ that matches operational screening practice (Section~\ref{sec:pauc}).
\item Two secondary contributions discussed in Section~\ref{sec:discussion}: a \textbf{tile-local Westfeld $\chi^2$ variant} that out-performs the global pairs-of-values detector by a wide margin on our carriers (Section~\ref{sec:tiled}), and a \textbf{mechanistic explanation of the $\chi^2$-spatial reversal} in terms of diffusion-decoder tone-curve smoothness (Section~\ref{sec:chi2spatial}).
\end{enumerate}

\paragraph{Outline.} Section~\ref{sec:relatedwork} reviews the relevant classical and synthetic-carrier literature. Section~\ref{sec:methods} describes the dataset, embedding pipeline, detectors, and statistical protocol. Section~\ref{sec:experiments} documents the experimental configuration. Section~\ref{sec:results} presents the results, RQ by RQ, with the supplementary partial-AUC analysis. Section~\ref{sec:discussion} discusses the findings and develops the two secondary contributions. Sections~\ref{sec:limitations}, \ref{sec:conclusion} and the Appendix cover limitations, conclusions, and reproducibility details.
...
In response to peer-review concerns that handcrafted detectors may underestimate detectability of modern caption-paired carriers, we additionally report a \emph{supplementary} analysis with two state-of-the-art learned detectors: SRNet~\cite{boroumand2019srnet} for the spatial branch and DCTR~\cite{holub2015dctr} for the frequency branch. Critically, both are trained on a 3{,}000-group \emph{caption-matched} training set disjoint from the test set by caption~ID -- not on BOSSBase~\cite{bas2011boss} -- so we avoid re-introducing the cover-source mismatch~\cite{giboulot2020csm,mallet2024csm} that BOSSBase-pretrained detectors suffer when applied to diffusion-generated images. The training protocol is described in Section~\ref{sec:learned-detectors}; results are reported in Section~\ref{sec:learned-results} alongside the classical contrasts.
...
\subsection{Supplementary Learned Detectors (SRNet, DCTR)}
\label{sec:learned-detectors}

Two state-of-the-art learned detectors are added as a supplementary analysis, one per embedding branch:

\paragraph{SRNet (spatial branch).} A deep residual network~\cite{boroumand2019srnet} with twelve convolutional blocks (Type 1$\times$2, Type 2$\times$5, Type 3$\times$4, Type 4$\times$1) and channel widths $\{64,16,16{\times}5,16,64,128,256,512\}$ matching the published Figure~2; final $512{\rightarrow}2$ fully-connected head. We faithfully include the Type~4 block's residual skip connection (some open-source ports omit it). Our implementation totals \TBD{srnet param count} trainable parameters; the paper claims 467k but Figure~2 composes to $\sim$4.9M and other open-source ports also land in the $2$--$5$M range. No TLU (paper-correct); BatchNorm throughout; Adam optimiser with learning rate $10^{-3}$ and reduce-on-plateau on validation AUC.

\paragraph{DCTR (frequency branch).} The 8{,}000-dimensional DCTR feature set of Holub \& Fridrich~\cite{holub2015dctr}: 64 DCT modes (full 8$\times$8 basis) applied via undecimated DCT, decomposed into 25 phase positions (the 5$\times$5 upper-left subset of the 8$\times$8 JPEG block alignment grid, defined as $(i\bmod 8, j\bmod 8)$), and a 5-bin absolute-truncated histogram (T$=$4) per (mode, phase) pair. Mode-specific quantization derived from the standard JPEG luminance table at Q$=$95. The classifier is a Fisher-LDA ensemble in the Kodovsk\'{y}-Fridrich style~\cite{fridrich2012srm}, implemented via scikit-learn's \texttt{BaggingClassifier(LinearDiscriminantAnalysis, n\_estimators=100, max\_features=0.1, bootstrap=True)} on the 8{,}000-dim standardised feature vectors.

\paragraph{Per-cell training.} Following steganalysis-literature standard we train one model per (method, payload) cell: three SRNet checkpoints (LSB / \{low, medium, high\}) and three DCTR ensembles (DCT / \{low, medium, high\}), six trained detectors total.

\paragraph{Training data and leakage guard.} The learned detectors are trained on a \emph{separate} 3{,}000-group caption-matched corpus (Section~\ref{sec:learned-results}); critically, the training caption pool excludes all 3{,}000 caption~IDs that appear in our test set. Inside the training set we use a group-aware split: $g\bmod 10 \in \{0..6\}$ go to train, $\{7,8\}$ to val, $\{9\}$ to an in-distribution held-out subset (only the train/val split is used for model selection; the held-out subset is reserved for sensitivity analysis). The training-set manifest is hashed (SHA-256, leading 16 hex digits) and the hash is embedded in every saved checkpoint; the inference scripts refuse to apply a checkpoint to any run whose manifest hash matches the training hash, programmatically preventing train/test leakage. Encryption is handled symmetrically with the classical analysis: at training time the paired sampler draws each cover with a randomly chosen encryption variant; at inference time each cover is scored exactly once and the score is fanned out to one row per encryption variant (the cover bytes are identical regardless of encryption, so a single score per cover is correct).

\paragraph{Reporting protocol.} The learned-detector contrasts are reported as \emph{supplementary} and do not enter the primary verdict table (Table~\ref{tab:verdicts}). They are computed with the same DeLong test, Bonferroni--Holm correction, and $\delta_\text{min}$ thresholds as the classical detectors, with their own 6 (method$\times$payload) strata per RQ.
...
\paragraph{Learned-detector training run.} A separate 3{,}000-group training run was executed on a rented RTX~4090 cloud instance to produce the SRNet and DCTR checkpoints (Section~\ref{sec:learned-detectors}). It uses the same factorial design as the test run (real + ML-A + ML-B carriers, LSB + DCT methods, three payload levels, two encryption modes) but draws caption~IDs from a pool that programmatically excludes every caption~ID in the test run's manifest. Training-run wall-clock was approximately \TBD{total hours}~h for $\$$\TBD{cost}; the resulting six checkpoints (3 SRNet + 3 DCTR), per-cell training logs, and the SHA-256 manifest hash of the training data are all included in the supplementary deliverables bundle.

\subsection{Artefacts Produced}
The $N\!=\!3{,}000$ test run produced 18{,}000 covers, 108{,}000 stego images, 648{,}000 detector predictions, 19 metrics CSVs, 61 figures, and the per-RQ verdicts in JSON and Markdown. Total wall-clock was approximately 13~hours, of which \textasciitilde6~h was cover generation (HF Inference API throughput-bound), \textasciitilde2~h was embedding, \textasciitilde0.5~h was detection, and the remainder was analysis. The unusually long analysis stage is a known consequence of an $O(N^2)$ Python ROC implementation, discussed in Section~\ref{sec:limitations}. The learned-detector inference adds \TBD{n} prediction rows (one per cover/stego per cell) to the existing 648{,}000-row \texttt{predictions.csv} via \texttt{merge\_learned\_predictions.py}; the analysis stage is unchanged.
...
The training corpus (3{,}000 caption-matched groups, disjoint from the test set by caption~ID) yielded \TBD{n train images} images after embedding (\TBD{n covers}~covers + \TBD{n stegos}~stego variants); the train/val split is \TBD{n train groups}/\TBD{n val groups} groups by $g\bmod10$. Training wall-clock on a single RTX~4090 was \TBD{srnet hours}~h for SRNet (3 cells, 60 epochs each) and \TBD{dctr hours}~h for DCTR (3 cells, parallel feature extraction across 18~CPU cores). Per-cell validation AUCs at the end of training are reported in Table~\ref{tab:learned-val-auc}.

\begin{table}[h]
\centering\small
\caption{Validation AUCs of the learned detectors at the end of training. The held-out validation set ($g\bmod10\in\{7,8\}$) is disjoint from the test set we score in Tables~\ref{tab:learned-headline-auc}--\ref{tab:learned-rq1}.}
...
\midrule
SRNet & LSB / low     & \TBD{auc} & \TBD{n}  \\
SRNet & LSB / medium  & \TBD{auc} & \TBD{n}  \\
SRNet & LSB / high    & \TBD{auc} & \TBD{n}  \\
DCTR  & DCT / low     & \TBD{auc} & \TBD{n}  \\
DCTR  & DCT / medium  & \TBD{auc} & \TBD{n}  \\
DCTR  & DCT / high    & \TBD{auc} & \TBD{n}  \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Headline AUCs on the held-out test set.} Applying the six trained checkpoints AS-IS to the 3{,}000-group test run (leakage-guarded by manifest hash; see Section~\ref{sec:learned-detectors}) yields the per-source AUCs in Table~\ref{tab:learned-headline-auc}.

\begin{table}[h]
\centering\small
\caption{Headline AUC on the held-out test run ($N\!=\!3{,}000$ groups), per carrier source and learned detector. ``Real'' aggregates COCO + Flickr30K; ``ML-A'' is SDXL; ``ML-B'' is FLUX.1-schnell.}
...
\midrule
SRNet & LSB / low     & \TBD{auc} & \TBD{auc} & \TBD{auc} \\
SRNet & LSB / medium  & \TBD{auc} & \TBD{auc} & \TBD{auc} \\
SRNet & LSB / high    & \TBD{auc} & \TBD{auc} & \TBD{auc} \\
DCTR  & DCT / low     & \TBD{auc} & \TBD{auc} & \TBD{auc} \\
DCTR  & DCT / medium  & \TBD{auc} & \TBD{auc} & \TBD{auc} \\
DCTR  & DCT / high    & \TBD{auc} & \TBD{auc} & \TBD{auc} \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{RQ1 (real vs.\ pooled ML) under learned detectors.} Restricting the DeLong-Bonferroni--Holm pipeline to the six learned-detector strata gives a pooled $\Delta_\text{AUC}\!=\!\TBD{val}$ with 95\% CI $\TBD{ci}$; the per-cell breakdown is in Table~\ref{tab:learned-rq1}.

\begin{table}[h]
\centering\small
\caption{RQ1 (real vs.\ pooled ML) under the learned detectors. Negative $\Delta_\text{AUC}$ means ML carriers are easier to attack than real photographs, consistent with the classical-detector finding.}
\label{tab:learned-rq1}
\setlength{\tabcolsep}{4pt}\renewcommand{\arraystretch}{1.1}
\begin{tabular}{@{}llrrr@{}}
\toprule
\textbf{Detector} & \textbf{Cell} & \textbf{$\Delta_\text{AUC}$} & \textbf{$p_\text{Holm}$} & \textbf{Sig.?} \\
\midrule
SRNet & LSB / low     & \TBD{val} & \TBD{p} & \TBD{?} \\
SRNet & LSB / medium  & \TBD{val} & \TBD{p} & \TBD{?} \\
SRNet & LSB / high    & \TBD{val} & \TBD{p} & \TBD{?} \\
DCTR  & DCT / low     & \TBD{val} & \TBD{p} & \TBD{?} \\
DCTR  & DCT / medium  & \TBD{val} & \TBD{p} & \TBD{?} \\
DCTR  & DCT / high    & \TBD{val} & \TBD{p} & \TBD{?} \\
\midrule
\textbf{Pooled}       & --- & \TBD{val} & --- & --- \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{RQ2 (SDXL vs.\ FLUX) and RQ4 (branch interaction).} The SDXL vs.\ FLUX contrast under the learned detectors yields a pooled $\Delta_\text{AUC}\!=\!\TBD{val}$ (95\% CI $\TBD{ci}$). The spatial$-$frequency branch contrast (SRNet against DCTR) yields $\Delta\Delta_\text{AUC}\!=\!\TBD{val}$, to be compared with the classical pooled estimate $-0.0013$ from Section~\ref{sec:rq4}.

\paragraph{RQ5 (encryption invariance).} The plain-vs-AES contrast under the learned detectors yields a pooled $\Delta_\text{AUC}\!=\!\TBD{val}$; all 6 strata fall \TBD{within / outside} the $\pm0.025$ equivalence margin. As with the classical detectors, this is the expected null: encryption changes payload bytes but not embedding distortion, so a detector that responds to distortion sees encrypted and unencrypted stegos identically.

\paragraph{Comparison with classical detectors.} Table~\ref{tab:learned-vs-classical} contrasts each learned detector against the strongest same-branch classical detector on the same payload cell.

\begin{table}[h]
\centering\small
\caption{Best-vs-best comparison: learned detector AUC vs.\ the strongest classical detector on the same (method, payload) cell. Positive $\Delta$ favours the learned detector.}
\label{tab:learned-vs-classical}
\setlength{\tabcolsep}{4pt}\renewcommand{\arraystretch}{1.1}
\begin{tabular}{@{}lllrrr@{}}
\toprule
\textbf{Cell} & \textbf{Learned} & \textbf{Best classical} & \textbf{Learn.\ AUC} & \textbf{Class.\ AUC} & \textbf{$\Delta$} \\
\midrule
LSB / low     & SRNet & \TBD{name} & \TBD{auc} & \TBD{auc} & \TBD{val} \\
LSB / medium  & SRNet & \TBD{name} & \TBD{auc} & \TBD{auc} & \TBD{val} \\
LSB / high    & SRNet & \TBD{name} & \TBD{auc} & \TBD{auc} & \TBD{val} \\
DCT / low     & DCTR  & \TBD{name} & \TBD{auc} & \TBD{auc} & \TBD{val} \\
DCT / medium  & DCTR  & \TBD{name} & \TBD{auc} & \TBD{auc} & \TBD{val} \\
DCT / high    & DCTR  & \TBD{name} & \TBD{auc} & \TBD{auc} & \TBD{val} \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Provenance.} The training-set caption manifest, model checkpoints, per-cell training logs, and the leakage-guard hash are all included in the supplementary deliverables bundle (\texttt{deliverables\_training\_v1.tar.gz}). The classical-detector predictions, the learned-detector predictions, and the verdict tables in this section are reproducible from the manifests + the seed (\texttt{4242}) in this bundle.
...
\paragraph{Scope bounds.} Two generator families only; we cannot disentangle architecture from training data in RQ2. Grayscale only; colour information is intentionally removed for closer comparability with BOSSBase~\cite{bas2011boss} and the published detectors, but a colour study would be informative. The \emph{primary} analysis uses classical (training-free) detectors only; learned baselines (SRNet and DCTR) are reported as a supplementary analysis in Section~\ref{sec:learned-results} and explicitly excluded from the primary verdict table. RQ4 pools an asymmetric set (three spatial vs.\ three frequency detectors after adding the tile-local variant); the branch contrast accounts for this in its inverse-variance weighting.
...
We ran a caption-matched 6-detector $\times$ 2-method $\times$ 3-payload $\times$ 2-encryption factorial on 3{,}000 caption groups (108{,}000 stego images, 648{,}000 detector predictions) to ask whether carrier source -- real photograph vs.\ diffusion-generated synthetic image -- changes the detectability of LSB-style steganography under matched embedding. Five of six training-free detectors agree that diffusion carriers are slightly easier to attack than photographs, with the gap concentrated at low payload (RQ1, RQ3); the two diffusion models we tested are interchangeable at the detector level (RQ2); the source effect is statistically similar across spatial and frequency embedding branches once pooled (RQ4); and AES-256-CBC encryption of the payload has no measurable detectability effect (RQ5). The supplementary partial-AUC analysis at FPR\,$\leq\!0.10$ amplifies every RQ1 effect by 3--7$\times$ and elevates the verdict from mixed to supported. The supplementary learned-detector analysis (SRNet for the spatial branch, DCTR for the frequency branch), trained on a separate 3{,}000-group caption-matched corpus disjoint from the test set by caption~ID, \TBD{corroborates / refines / contradicts} the classical-detector RQ1 finding and yields pooled $\Delta_\text{AUC}\!=\!\TBD{val}$ on the learned strata.
