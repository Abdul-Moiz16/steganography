...
We propose five experiments to establish the tile-local detector beyond the present corpus.  A reference implementation of the experiment runners is provided in the supplementary code under \texttt{scripts/experiments/tiled\_chi2\_validation/}; each is self-contained, runs on the existing test corpus or on a freshly downloaded BOSSBase~\cite{bas2011boss}, and writes a single CSV plus a small figure.
