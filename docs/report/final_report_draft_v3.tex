...
\noindent\textbf{Abstract}---We study whether the source of a carrier image -- real photograph vs.\ diffusion-generated -- changes the detectability of LSB-style steganography under fixed embedding settings. A caption-matched factorial design ($N\!=\!3{,}000$ groups across real, SDXL and FLUX.1-schnell carriers; spatial LSB and DCT-LSB at three payload levels) is scored by six training-free statistical detectors and analysed with paired DeLong tests, Bonferroni--Holm correction, and a pre-registered practical-significance gate. Five of six classical detectors find diffusion carriers \emph{easier} to attack than real photographs; the sixth ($\chi^2$-spatial) reverses, an artefact that dissolves under partial-AUC at operational FPR\,$\leq\!0.10$. SDXL vs.\ FLUX shows no practical difference, and AES-256-CBC encryption is perfectly invariant. We cross-validate every pre-registered finding on an orthogonal detector family (SRNet, DCTR) trained on a disjoint caption-matched corpus: RQ2/4/5 corroborate directly, RQ3 refines (same monotone direction at $\sim$half magnitude), and RQ1 splits by detector branch -- DCTR replicates the classical real-harder pattern at $\Delta_\text{AUC}\!\approx\!-0.03$; SRNet, saturated at AUC$\geq$0.997, does not (an ambiguity that a real-only-training ablation, V2a, is designed to resolve). Two secondary contributions: a tile-local Westfeld $\chi^2$ variant that becomes the strongest frequency-branch classical detector in our pipeline, and a mechanistic account of the $\chi^2$-spatial reversal grounded in diffusion-decoder tone-curve smoothness.
