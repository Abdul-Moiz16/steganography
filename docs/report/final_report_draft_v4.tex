...
\noindent\textbf{Abstract}---This article asks whether the source of a carrier image -- a real photograph or a diffusion generated one -- changes how detectable LSB steganography is under fixed embedding settings. We evaluated a corpus of 3{,}000 caption-matched image groups across three carrier sources (real, SDXL, FLUX.1-schnell), two embedding branches (spatial LSB on PNG, JSteg-style DCT-LSB on JPEG~Q=95), three payload levels, and two payload conditions (plain and AES-256-CBC). Each image was scored by six training-free statistical detectors and the results analysed with paired DeLong tests under Bonferroni--Holm correction. Five of the six detectors found diffusion carriers easier to attack than photographs; the sixth ($\chi^2$-spatial) reversed this, an effect that disappears under partial AUC at operational false-positive rates. We then cross-validated the pre-registered protocol on a fundamentally different class of detectors: a deep residual network (SRNet) and an undecimated-DCT feature extractor with an LDA ensemble (DCTR), both trained on a separate, matched dataset, in two configurations: \textbf{matched training} (training mix matches the test mix, 1:1:1 real/SDXL/FLUX) and \textbf{real-only training} (AI carriers held out, the realistic deployment scenario). The matched-training cross-validation replicated four of five findings; the matched-vs-real-only contrast showed that SRNet's apparent source-invariance under matched training comes from the balanced training mix rather than anything architectural -- removing AI carriers from training opens a $0.26$--$0.45$~AUC gap on the same AI test carriers, confirming the carrier-source effect for deep CNNs once they have to handle sources they were never trained on. Two further contributions: a tile-local Westfeld~$\chi^2$ variant that becomes the strongest frequency-branch classical detector on our test set, and a mechanistic account of the $\chi^2$-spatial reversal grounded in the tone-curve smoothness of diffusion-decoder outputs.
...
\item A mechanistic account of the $\chi^2$-spatial reversal -- the single detector that reports the opposite carrier-source ordering from its peers -- grounded in the tone-curve smoothness of diffusion-decoder outputs, with a falsification of the naive smoothness hypothesis via the cover-side pairs-of-values imbalance measurement (Section~\ref{sec:chi2spatial}).
\end{enumerate}
...
In response to peer-review concerns that handcrafted detectors may underestimate the detectability of modern caption-paired carriers, we also report a \emph{cross-validation} on two state-of-the-art learned detectors: SRNet~\cite{boroumand2019srnet} for the spatial branch and DCTR~\cite{holub2015dctr} for the frequency branch. Both are trained on a separate, matched dataset of 3{,}000 caption-matched image groups disjoint from the test set by caption~ID -- not on BOSSBase~\cite{bas2011boss} -- so we avoid re-introducing the cover-source mismatch~\cite{giboulot2020csm,mallet2024csm} that BOSSBase-pretrained detectors suffer when applied to diffusion generated images. The training protocol is described in Section~\ref{sec:learned-detectors}; cross-validation results are reported in Section~\ref{sec:learnedxval} for two complementary training configurations (\emph{matched} with all carrier sources present in training, and \emph{real-only} with AI carriers held out), with the second isolating whether the spatial-CNN's apparent source-invariance under matched training is architectural or a training-balance artefact (Section~\ref{sec:v2a}).
...
\paragraph{Real-only-training ablation.} The primary learned-detector training -- \emph{matched training}, file label \texttt{V1} -- uses the same 3-source carrier balance (real + SDXL + FLUX, 1:1:1) as the test set, so SRNet sees all carrier types during training. Any source-invariance observed at inference time could then be either an architectural property of the CNN or an artefact of balanced training exposure. A second training run -- \emph{real-only training}, file label \texttt{V2a} -- trains the same six checkpoints on a real-only corpus of disjoint Flickr30k captions, holding sample count constant; the resulting source-AUC pattern at test time disentangles the two interpretations (\S\ref{sec:v2a}).
...
\paragraph{Bridge to cross-validation.} The pre-registered six-detector verdicts are complete. Section~\ref{sec:learnedxval} now re-runs the same DeLong + Bonferroni--Holm + $\delta_\text{min}$ protocol on a fundamentally different class of detectors (two learned baselines, SRNet and DCTR) trained on a separate, matched dataset, in two complementary configurations: \emph{matched training} (the pre-registered learned cross-validation) and \emph{real-only training} (an in-paper ablation that holds AI carriers out of training to address how training-set composition shapes the learned-detector findings).
...
\begin{itemize}
\item \textbf{Matched training} (file label \texttt{V1}): the training corpus's carrier-source distribution matches the test corpus's (real + SDXL + FLUX, 1:1:1). \emph{Question answered}: given equal training exposure to all carrier sources, do learned detectors show source-correlated performance differences on the test set?
\item \textbf{Real-only training} (file label \texttt{V2a}): the training corpus is restricted to real Flickr30k captions; the AI sources are held out from training but still appear in the test set. \emph{Question answered}: if the detector is trained without seeing the AI carrier sources at all (the realistic deployment scenario where AI carriers post-date the training corpus), does it still generalise to them?
\end{itemize}
...
Throughout this section we use the descriptive names; the \texttt{V1}/\texttt{V2a} labels survive in the supplementary file paths (\texttt{runs/.../learned\_shadow/} vs \texttt{learned\_shadow\_v2a/}, \texttt{models/training\_v1/} vs \texttt{training\_v2a/}) for backwards compatibility with the reproduction artefacts.
...
\begin{table}[!htbp]
\centering\footnotesize
\caption{Matched-training (V1) learned-detector AUC on the test run, per carrier source. ML-A$=$SDXL, ML-B$=$FLUX. Encryption averaged within each cell (per-encryption agree to $\leq\!0.001$). Overall: SRNet 0.998, DCTR 0.814.}
\label{tab:learned-headline-auc}
\setlength{\tabcolsep}{5pt}\renewcommand{\arraystretch}{1.05}
\begin{tabular}{@{}llrrr@{}}
\toprule
\textbf{Det.} & \textbf{Cell} & \textbf{Real} & \textbf{ML-A} & \textbf{ML-B} \\
\midrule
...
\begin{table}[!htbp]
\centering\footnotesize
\caption{Matched-training (V1) cross-validation verdicts. Classical and learned $\Delta_\text{AUC}$ inverse-variance pooled within each detector family.}
...
\midrule
RQ1 real vs.\ ML        & $-0.0129$ mixed     & $+0.0012$ trivial   & \textbf{Contradicts$^\star$} \\
RQ2 SDXL vs.\ FLUX      & $+0.0004$ trivial   & $-0.0002$ trivial   & Corroborates \\
RQ3 payload$\times$src  & $-0.0157$ supp.     & $-0.0068$ mixed     & Refines (1/2 mag.) \\
RQ4 spatial vs.\ freq.  & $-0.0013$ trivial   & $+0.0159$ trivial   & Corroborates \\
RQ5 encr.\ invariance   & $\approx 0$ supp.   & $\approx 0$ supp.   & \textbf{Corroborates} \\
\bottomrule
\end{tabular}\\[1pt]
{\scriptsize $^\star$\,pooling artefact -- see \S\ref{sec:disc-rq1-revisited}.}
\end{table}
...
\end{tikzpicture}
\caption{Cross-validation verdicts visualised. Each row is one RQ; dark dot is the classical pooled $\Delta_\text{AUC}$, orange dot is the matched-training-learned (V1) pooled $\Delta_\text{AUC}$. RQ3 keeps direction at half the magnitude; RQ2/4/5 stack near zero; RQ1 sign-flips at small magnitude -- the within-family Figure~\ref{fig:within-family} below resolves this as a pooling artefact, not a genuine reversal. A real-only-training (V2a) panel of the same chart appears as Figure~\ref{fig:verdict-matrix-three-way}.}
...
\centering
\begin{tikzpicture}[
  % Heat-cell colour: linear interpolation from red (low) to green (high)
  % via the umdark/umlight/umorange palette is not natural, so use a
  % brand-tinted red->amber->green ramp:
  %   AUC 0.50 -> umorange   (poorest)
  %   AUC 0.75 -> umorange!40
  %   AUC 0.90 -> umlight!50
  %   AUC 1.00 -> umlight   (best)
  cellbase/.style={draw=umdark!70, line width=0.4pt},
]
% Helper: given AUC value v, define a fill colour name and text colour.
% We use TikZ \pgfmathsetmacro to map and let user pick a colour for each cell.
% Two 6x3 grids side by side: rows = (detector x payload), cols = (real, ml_a, ml_b).
% Cell w x h = 1.2 x 0.55 cm; row label takes the leftmost 1.7 cm.

\def\cw{1.2}   % cell width
\def\ch{0.55}  % cell height
\def\rowlbl{1.65} % row-label width
\def\panelgap{0.7}

\foreach \panel/\panelX/\title in {0/0/{Matched training (V1)}, 1/{8.95}/{Real-only training (V2a)}} {
  \pgfmathsetmacro{\px}{\panelX}
  \node[font=\footnotesize\bfseries\color{umdark}] at (\px+\rowlbl+1.5*\cw, 3.7*\ch+0.65) {\title};
  % Column headers
  \foreach \j/\src in {0/Real, 1/SDXL, 2/FLUX} {
    \pgfmathsetmacro{\xx}{\px+\rowlbl+\j*\cw+\cw/2}
    \node[font=\scriptsize\color{umdark}] at (\xx, 3.2*\ch) {\src};
  }
}

% Per-cell data: panel, row, col, AUC value.  Panel 0 = matched (V1), Panel 1 = real-only (V2a).
% Row order: SRNet/low, SRNet/medium, SRNet/high, DCTR/low, DCTR/medium, DCTR/high (top->bottom).
\foreach \panel/\panelX in {0/0, 1/{8.95}} {
  \pgfmathsetmacro{\px}{\panelX}
  % Row labels (panel 0 only, shared visually)
  \ifnum\panel=0
    \foreach \r/\lbl in {0/{SRNet / low}, 1/{SRNet / medium}, 2/{SRNet / high},
                          3/{DCTR / low}, 4/{DCTR / medium}, 5/{DCTR / high}} {
      \pgfmathsetmacro{\yy}{(2.5-\r)*\ch}
      \node[font=\scriptsize\color{umdark}, anchor=east] at (\px+\rowlbl-0.05, \yy) {\lbl};
    }
  \fi
}

% Cell data: panel, row, col, AUC.
\foreach \p/\r/\c/\v in {%
  ...
  \pgfmathsetmacro{\panelX}{\p*8.95}
  \pgfmathsetmacro{\xx}{\panelX+\rowlbl+\c*\cw}
  \pgfmathsetmacro{\yy}{(2.5-\r)*\ch-\ch/2}
  % Map AUC to colour: <=0.55 -> heavy umorange; 0.55-0.75 -> umorange!~50; 0.75-0.90 -> umlight!50; 0.90-1.0 -> umlight strong.
  % Compute a "good-ness" intensity 0..100 from AUC in [0.5, 1.0]:
  \pgfmathsetmacro{\pct}{int(100*(\v-0.5)/0.5)}
  % If v < 0.7 use umorange-shaded, else umlight-shaded.
  \pgfmathsetmacro{\isLow}{ifthenelse(\v<0.70, 1, 0)}
  \pgfmathsetmacro{\textcolor}{ifthenelse(\v<0.70||\v>0.95, 0, 1)}
  \ifdim\v pt<0.70pt
    \pgfmathsetmacro{\fillpct}{int(85-(\v-0.5)*180)} % 0.5 -> 85, 0.7 -> 49
    \fill[umorange!\fillpct, cellbase] (\xx,\yy) rectangle (\xx+\cw,\yy+\ch);
  \else
    \pgfmathsetmacro{\fillpct}{int(20+(\v-0.7)*200)} % 0.7 -> 20, 1.0 -> 80
    \fill[umlight!\fillpct, cellbase] (\xx,\yy) rectangle (\xx+\cw,\yy+\ch);
  \fi
  \draw[cellbase] (\xx,\yy) rectangle (\xx+\cw,\yy+\ch);
  \ifdim\v pt<0.70pt
    \node[font=\scriptsize\color{umdark}, anchor=center] at (\xx+\cw/2,\yy+\ch/2) {\v};
  \else
    \node[font=\scriptsize\color{umdark}, anchor=center] at (\xx+\cw/2,\yy+\ch/2) {\v};
  \fi
}

% Bottom caption-line: colour bar legend
\begin{scope}[shift={(0,-1.0)}]
  \node[font=\scriptsize\color{umdark}, anchor=east] at (0.0,0) {AUC scale:};
  \foreach \i/\auc in {0/0.5, 1/0.6, 2/0.7, 3/0.8, 4/0.9, 5/1.0} {
    \pgfmathsetmacro{\xx}{0.15 + \i*1.7}
    \ifdim\auc pt<0.70pt
      \pgfmathsetmacro{\pct}{int(85-(\auc-0.5)*180)}
      \fill[umorange!\pct] (\xx,-0.18) rectangle (\xx+1.6,0.18);
    \else
      \pgfmathsetmacro{\pct}{int(20+(\auc-0.7)*200)}
      \fill[umlight!\pct] (\xx,-0.18) rectangle (\xx+1.6,0.18);
    \fi
    \draw[umdark!50, line width=0.3pt] (\xx,-0.18) rectangle (\xx+1.6,0.18);
    \node[font=\scriptsize\color{umdark}] at (\xx+0.8,-0.42) {\auc};
  }
\end{scope}
\end{tikzpicture}
\caption{Per-(detector, payload, source) test AUC under matched (V1, left) vs real-only (V2a, right) training. Cells are coloured by AUC (orange-tinted below 0.70 indicating poor detection, blue-tinted above 0.70 indicating good detection). Under matched training every cell is high; under real-only training the SRNet ml\_a / ml\_b cells at medium and high payload collapse to orange (AUC 0.55--0.71), while the real column stays blue (AUC 0.99). DCTR shows the same pattern at smaller absolute magnitude. The heatmap form makes the cover-source-mismatch effect visually immediate: orange blocks materialise only in the columns the model never saw at training time.}
\label{fig:matched-vs-realonly-heatmap}
\end{figure}

\begin{table}[!htbp]
\centering\footnotesize
\caption{Real-only training (V2a): per-(detector, payload) validation AUC and per-source test AUC at the plain-encryption cell. ``Val'' is the held-out-of-training-corpus AUC; the three source columns are AUC on the held-out test corpus (108{,}000 stegos $+$ covers). DCTR also carries an out-of-bag estimate $E_\text{OOB}$ of the minimum total detection error at equal priors, computed by the same FLD-ensemble convention as Holub \& Fridrich~\cite{holub2015dctr} Table~V.  See Figure~\ref{fig:dctr-eoob-ladder} for the visualisation.}
\label{tab:v2a-results}
\setlength{\tabcolsep}{4pt}\renewcommand{\arraystretch}{1.05}
\begin{tabular}{@{}llccccc@{}}
\toprule
\textbf{Det.} & \textbf{Payload} & \textbf{Val} & \textbf{$E_\text{OOB}$} & \textbf{Real} & \textbf{ML-A} & \textbf{ML-B} \\
\midrule
...
\begin{table}[!htbp]
\centering\footnotesize
\caption{Matched-training (V1: 1:1:1 real/SDXL/FLUX) vs.\ real-only training (V2a) -- per-source test AUC at the plain-encryption cell, contrasted directly. The matched-training numbers are the same predictions used in Table~\ref{tab:learned-headline-auc} (Section~\ref{sec:learnedxval}) and Figure~\ref{fig:within-family}. Bold entries mark drops $\geq 0.10$ from matched to real-only training -- the diagnostic of training-balance dependence.}
\label{tab:v1-vs-v2a}
\setlength{\tabcolsep}{3pt}\renewcommand{\arraystretch}{1.05}
\begin{tabular}{@{}ll | ccc | ccc@{}}
\toprule
& & \multicolumn{3}{c}{\textbf{matched (V1)}} & \multicolumn{3}{c}{\textbf{real-only (V2a)}} \\
\textbf{Det.} & \textbf{Payload} & Real & ML-A & ML-B & Real & ML-A & ML-B \\
\midrule
...
\begin{scope}[shift={(0.4, -1.3)}]
  ...
  \node[font=\scriptsize\color{umdark}, right=3pt] at (0.32,-0.34) {test-set $P_E^{\min}$ (real + ML carriers, OOD on ML)};
...
\end{tikzpicture}
\caption{DCTR under real-only training: three error metrics by payload. $E_\mathrm{OOB}$ (dark) is the training-time out-of-bag ensemble residual (Holub \& Fridrich~\cite{holub2015dctr} convention, computed post-training by deterministic re-fit). Test-set $P_E^{\min}$ (orange) is the operational detection error on the held-out test corpus (real + ML carriers). $1-\mathrm{val\_AUC}$ (light) is a held-out-of-training proxy for $P_E$ on the in-distribution carriers (real only). Test-set $P_E^{\min}$ is uniformly worse than $E_\mathrm{OOB}$ because the test corpus contains ML carriers the real-only-trained DCTR never saw -- the cover-source-mismatch effect manifest at the operational metric.}
...
% with an arrow connecting them showing the "shift" (which collapses on ML carriers).
...
\end{scope}
\end{tikzpicture}
\caption{Real-only-trained SRNet, medium payload, plain encryption: cover-mean and stego-mean score per carrier source, with the arrow showing the cover-to-stego shift. On real carriers the shift is 0.95 (clean separation, AUC 0.986). On SDXL the model assigns covers a near-1.0 stego score because diffusion-decoder noise patterns trigger the LSB-perturbation detector; the shift collapses to 0.05 and AUC drops to 0.547. On FLUX the same mechanism applies more weakly (shift 0.21, AUC 0.644). Per-payload, low-payload AUC stays higher (0.88--0.93) because the tighter decision boundary of the low-payload checkpoint still admits separation; the collapse only manifests at medium / high payload, where the model has learned features that respond to high-frequency content in general. This is structurally consistent with the $\chi^2$-spatial cover-side variance-inflation of Section~\ref{sec:chi2spatial}: both detectors register diffusion-decoder noise as a stego-like signal on the cover side.}
\label{fig:v2a-srnet-mechanism}
\end{figure}

\paragraph{Implication for RQ1.} Re-running the cross-validation pipeline on the real-only predictions (\texttt{runs/.../learned\_shadow\_v2a/}) flips four of the five RQ verdicts at the learned-detector family level: RQ1 from trivial ($\Delta\!=\!+0.0012$) to supported with $\Delta\!=\!+0.2473$ (CI $[+0.245,+0.250]$); RQ2 from trivial to supported with $\Delta\!=\!-0.0689$; RQ4 (spatial vs.\ frequency branch contrast) from trivial to supported with $\Delta\!=\!+0.2195$; RQ3 retains the mixed verdict but the per-payload real-minus-ML gaps grow from $\{-0.014, -0.013, -0.007\}$ to $\{+0.053, +0.220, +0.213\}$. RQ5 (encryption invariance) is unchanged at $\Delta\!\approx\!0$ as expected -- an encryption ablation does not depend on training-set carrier composition.

\paragraph{Mechanism of the real-only medium/high-payload collapse on ML carriers.} Real-only-trained SRNet on ML carriers is worst at medium payload (AUC 0.55 on ML-A) and recovers only partially at high payload (0.59), rather than improving monotonically with payload as the standard expectation predicts. A per-(payload, source, label) decomposition of the SRNet score distribution (Figure~\ref{fig:v2a-srnet-mechanism}) explains this directly: at medium and high payload, the real-only-trained SRNet assigns ML \emph{covers} a near-saturated stego score (cover-mean 0.79--0.95) almost indistinguishable from the score it assigns ML \emph{stegos} (stego-mean 1.000); cover-stego separation collapses to a shift of 0.05--0.27 on the [0,1] score axis and AUC drops accordingly. On real carriers the same checkpoint behaves normally: cover-mean 0.030, stego-mean 0.978, shift 0.95. The real-only-trained medium checkpoint was trained to discriminate ``real cover'' from ``real cover $+$ 15\,\% LSB perturbation,'' and at test time it interprets ML covers' diffusion-decoder noise patterns as if they were the perturbation signal it was trained to detect. The same effect appears more weakly at high payload (cover-mean 0.91 on ML-A) and is largely absent at low payload (cover-mean 0.16 on ML-A) -- the lower-payload checkpoint's tighter decision boundary still admits separation. This is structurally consistent with the cover-side variance-inflation mechanism we identify for the $\chi^2$-spatial detector (Section~\ref{sec:chi2spatial}): both detectors register diffusion-decoder noise as a stego-like signal on the cover side, and both consequently lose discrimination power on ML carriers in ways the same detectors applied to real carriers do not. A finer-grained analysis of which SRNet layer activations drive this is a natural follow-up.
...
\paragraph{Provenance.} The training-set caption manifests (matched and real-only; file labels V1 and V2a), model checkpoints, per-cell training logs, and the leakage-guard SHA-256 hashes are included in the supplementary deliverables bundle. The classical-detector predictions, the matched- and real-only-trained learned-detector predictions, and the verdict tables in this section are all reproducible from the manifests $+$ the seed (\texttt{4242}).
...
\end{enumerate}
We read the corrected verdict as an \emph{honesty} signal: there is no universally preferable branch for distinguishing real from ML carriers; the dominant branch depends on payload. This is a more useful operational message than ``spatial is better'' or ``frequency is better'' would have been, because it tells a deployer what to optimise for in each payload regime.

\subsection{Synthesis Across the Four Contributions}

The five-RQ classical analysis (\S\ref{sec:results-classical}) and the learned-detector cross-validation (\S\ref{sec:learnedxval}) together establish that the real-vs-diffusion carrier-source effect is real, small in pooled magnitude, sharply concentrated at low payload and at the operational left-tail of the ROC, and \emph{detector-family dependent} -- present and direction-consistent in three of four classical detectors and in the hand-crafted DCT learned detector, abstracted away in the deep spatial CNN, and reversed in one classical detector that under-performs its peers on a separate axis. The two detector-side findings of Section~\ref{sec:detector-side} are not incidental: the tile-local $\chi^2$-DCT proposal (\S\ref{sec:tiled}) is what \emph{flipped} the RQ4 branch contrast from ``mixed'' at $N=1{,}000$ to ``trivial'' at $N=3{,}000$ by tightening the frequency-branch average, and the $\chi^2$-spatial reversal analysis (\S\ref{sec:chi2spatial}) is what reduced an apparent ``contradicts'' finding to an artefact of operationally-irrelevant high-FPR behaviour. Detector engineering and the substantive scientific finding are therefore not separable contributions in this corpus -- they are two sides of the same investigation.

% ============================================================
\section{Detector-Side Findings}
\label{sec:detector-side}
% ============================================================

The five RQs and the cross-validation evaluate the carrier-source effect using detectors that we treat as fixed instruments.  Two by-products of that evaluation are themselves contributions to the steganalysis literature, independent of the carrier-source question: a tile-local variant of the Westfeld pairs-of-values test that out-performs every classical DCT detector in our pipeline (\S\ref{sec:tiled}), and a mechanistic account of why the $\chi^2$-spatial detector inverts the carrier-source ordering reported by every other detector in our suite (\S\ref{sec:chi2spatial}).
...
\paragraph{Caveats.} Three properties of the present evaluation bound the generality of the result.  \textit{(i)}~Max-pooling raises false-positive risk: the detector's score on a cover image is the noisiest of $T^2$ $\chi^2$ samples, which can exceed the global $\chi^2$ on the same image.  Our paired-AUC framework controls for this in the within-corpus contrast but a deployed setting would need explicit recalibration against natural-image false-positive rates.  \textit{(ii)}~Tile-size dependence is untested in this paper: we did not sweep $T$.  \textit{(iii)}~The tile-local detector is the only DCT-branch detector that retains a practical-significance signal on ML carriers at low payload (\S\ref{sec:rq1}), so the gain may be specific to the spatial-heterogeneity regime characteristic of diffusion-decoder outputs and not necessarily generalise to BOSSBase-style camera covers.

\paragraph{Recommended validation experiments.}
\label{sec:tiled-validation}
We propose five experiments to establish the tile-local detector beyond the present corpus.  A reference implementation of the experiment runners is provided in the supplementary code under \texttt{scripts/experiments/tiled\_chi2\_validation/}.  Each script is self-contained and accepts an arbitrary test-run directory via \texttt{--run}; both the present caption-matched corpus and a BOSSBase~\cite{bas2011boss} import (via the bundled \texttt{import\_bossbase.py} helper) are first-class targets.  Every experiment reports two metrics in its results CSV: ROC-AUC (consistent with the present paper) and $P_E^\text{min} = \tfrac{1}{2}\min_\tau (\text{FPR}(\tau) + \text{FNR}(\tau))$, the operational error rate reported by the Fridrich classical-detector papers~\cite{fridrich2001lsb,fridrich2003calib} and by the DCTR evaluation~\cite{holub2015dctr} as the trained-ensemble estimate $E_\text{OOB}$.  Reporting both makes the results directly cross-comparable to either body of literature.

\begin{enumerate}
\item \textbf{Tile-size sweep ($T$-sweep).} Compare $T\!\in\!\{1,2,3,4,6,8\}$ on the present test corpus and (if available) on BOSSBase.  $T\!=\!1$ recovers the global Westfeld detector; an interior optimum at $T\!=\!2$ or $T\!=\!3$ would confirm that the gain is not monotone with finer tiling.  Expected output: AUC vs.\ $T$ at each (method, payload) cell.
\item \textbf{Cross-payload tile-size invariance.}  Repeat experiment~1 stratified by payload level.  An optimal $T$ that drifts with payload would indicate that the tile size is selecting a particular spatial frequency of carrier heterogeneity rather than a universal grain.
\item \textbf{Pooling-rule ablation.}  Replace max-pooling with mean, top-$k$ mean, and median.  Max-pooling is the natural choice for a sparse-localised stego signal; mean is the natural choice if heterogeneity averages out.  An ablation reveals which assumption the data supports.
\item \textbf{Comparison against literature baselines.}  On the same BOSSBase cells, evaluate the tile-local $\chi^2$ alongside the sliding-window $\chi^2$ of Westfeld and Pfitzmann~\cite{westfeld1999chi} and a rich-model baseline (SRM~\cite{fridrich2012srm} or DCTR~\cite{holub2015dctr}).  Positions the contribution against published precedent.
\item \textbf{JPEG-quality sweep.}  Re-embed the test covers at $Q\!\in\!\{75,85,90,95,98\}$ and re-score with both global and tile-local detectors.  Tests whether the gain is JPEG-quality-dependent.  Most expensive of the five (requires re-embedding) but the only one that probes the standard pipeline-design dimension.
\end{enumerate}

Each experiment is independently runnable; the entry-point script writes a status block to \texttt{logs/tiled\_validation/} on completion.  The five together form a coherent follow-up study of the tile-local proposal at roughly 1 day of laptop CPU plus a small Vast.ai instance for the JPEG-quality sweep.
...
\paragraph{Embedding-family scope.} Every result in this paper is reported for \emph{non-adaptive} LSB-replacement embeddings: spatial LSB on PNG and JSteg-style DCT-LSB on JPEG~Q=95. Modern content-adaptive JPEG embeddings such as J-UNIWARD~\cite{holub2014uniward}, S-UNIWARD~\cite{holub2014uniward}, UED~\cite{guo2014ued} and their spatial-domain counterparts (HILL~\cite{li2014hill}, MiPOD~\cite{sedighi2016mipod}) are not tested. This is a deliberate design choice with two consequences. \emph{(i)~Detector-side:} the $\chi^2$-family detectors that drive five of our six pre-registered findings are obsoleted \emph{by construction} on adaptive embeddings -- the pairs-of-values signature they target is precisely what an adaptive distortion function is designed to preserve -- so an apples-to-apples comparison across the $\chi^2$ family requires a non-adaptive embedding, and the tile-local $\chi^2$-DCT contribution (Section~\ref{sec:tiled}) is intrinsically scoped to that family. \emph{(ii)~Carrier-source:} fixing the embedding to a content-agnostic rule isolates the carrier as the experimental variable; an adaptive embedding's distortion function would itself respond to carrier statistics (textured regions are preferred and diffusion outputs differ from photographs in their texture distribution), entangling ``real-vs-ML detectability gap'' with ``embedding-adapted-to-carrier'' in ways the present factorial cannot disentangle. Our findings therefore do not transfer to adaptive embeddings without re-testing; the orthogonal axis -- AI carriers under adaptive embeddings -- has been studied by M\'ereur~\textit{et~al.}~\cite{mereur2024deepfake} using HILL / S-UNIWARD / MiPOD at 0.4~bpp against an SRMQ1 + low-complexity-linear-classifier feature set and an EfficientNet-v2 deep classifier on a 15-generator pool (GAN- and diffusion-based, including SDXL but not FLUX). The two studies are complementary along three axes: detector family (training-free $\chi^2$ + caption-matched-trained SRNet/DCTR here vs.\ feature-set-trained classifiers there), embedding family (non-adaptive LSB here vs.\ adaptive STC-coded distortion-minimisation there), and dataset design (caption-matched factorial here vs.\ unpaired generator pools there); together they bracket the deepfake-as-carrier question across detector training regime and embedding adaptivity.

\paragraph{Learned-detector caveats.} Three caveats accompany the supplementary learned-detector analysis. \textit{(i)} A single training-set random seed (\texttt{4242}) is used for both matched and real-only training; we do not report multi-seed variability of the learned-AUC estimates. The headline real-only finding -- 0.26--0.45 AUC drop on AI carriers when SRNet is trained real-only -- rests on a large, mechanistically explained structural effect (\S\ref{sec:v2a}: cover-side score saturation on ML carriers when the trained checkpoint interprets diffusion noise as LSB perturbation) and is not expected to be seed-dependent in direction; the absolute magnitude may shift by a few hundredths across seeds. A three-seed re-run of the real-only SRNet training would tighten the AUC point estimates with error bars but would not change the qualitative conclusion. \textit{(ii)} The DCTR feature extractor uses the 8{,}000-dim variant of Holub \& Fridrich~\cite{holub2015dctr} interpreted as 64 modes $\times$ 25 phases (5$\times$5 alignment subset) $\times$ 5 truncated bins; the exact phase decomposition that yields the paper's headline 8{,}000 count is somewhat ambiguous from the paper text alone, and our interpretation is one of several defensible reconstructions. \textit{(iii)} SRNet is trained on caption-matched data, not on BOSSBase; this is the academically preferable choice for our cover-source-mismatch-aware design, but it makes our AUC numbers not directly comparable to literature SRNet figures on BOSSBase.
...
We ran a study across 6 detectors, 2 embedding methods, 3 payload levels, and 2 encryption conditions on 3{,}000 caption-matched image groups (108{,}000 stego images, 648{,}000 classical-detector predictions) to ask whether the carrier source -- real photograph vs.\ a diffusion generated one -- changes how detectable LSB steganography is under matched embedding. Five of six training-free detectors agree that diffusion carriers are slightly easier to attack than photographs, with the gap concentrated at low payload (RQ1, RQ3); the two diffusion models we tested are interchangeable at the detector level (RQ2); the source effect is statistically similar across spatial and frequency embedding branches once pooled (RQ4); and AES-256-CBC encryption of the payload has no measurable detectability effect (RQ5). The partial AUC analysis at FPR\,$\leq\!0.10$ amplifies every RQ1 effect by 3--7$\times$ and lifts the verdict from mixed to supported. We then cross-validated on two learned baselines from a fundamentally different class of detectors (SRNet for the spatial branch, DCTR for the frequency branch), both trained on a separate, matched dataset in two complementary configurations -- \emph{matched training} (1:1:1 real/SDXL/FLUX) and \emph{real-only training} (AI carriers held out). Matched training corroborates RQ2, RQ4 and RQ5 directly, refines RQ3 to roughly half the classical magnitude in the same direction, and clarifies RQ1: DCTR (a hand-crafted frequency feature set scored by an LDA ensemble) reproduces the classical real-harder pattern at $\Delta_\text{AUC}\!\approx\!-0.030$ within its branch, while SRNet (a deep spatial CNN, saturated at AUC $\geq 0.997$ under matched training) abstracts the source signal away. The matched-vs-real-only contrast (\S\ref{sec:v2a}) shows that this matched-training source-invariance comes from the balanced training mix rather than anything architectural: SRNet trained without AI carriers in its training mix loses 0.26--0.45~AUC on the ML test sources, exposing a substantial carrier-source generalisation gap once the training-mix protection is removed.
